...
@section('title', 'Off Time Requests')
...
@section('content')
    ...
    <div class="breadcrumb-card">
        ...
        <a href="{{ route('off-time-requests.index') }}">Off Time Requests</a>
        <span class="divider">/</span>
    ...
    <div class="main-card">
        ...
        {{-- FILTER BAR --}}
        <form method="GET" action="{{ route('off-time-requests.index') }}" style="display: flex; flex-wrap: wrap; gap: 12px; align-items: center; margin-bottom: 24px;">
            {{-- Category Filter --}}
            <select name="category" class="filter-select" style="min-width: 170px; height: 38px;">
                <option value="all" {{ empty($category) || $category === 'all' ? 'selected' : '' }}>All Categories</option>
            ...
            @if ($search || ($status && $status !== 'all') || ($category && $category !== 'all') || $date)
                <a href="{{ route('off-time-requests.index') }}" class="btn-reset-gray" style="height: 38px; padding: 0 16px; display: inline-flex; align-items: center;">
                    Reset Filter
                </a>
            @endif
        </form>
        ...
        <div class="table-responsive">
            <table class="data-table" id="off-time-requests-table">
                <thead>
                    <tr>
                ...
                <tbody>
                    @forelse ($offTimeRequests as $index => $req)
                        <tr>
                            <td style="color: #94a3b8; font-size: 13px;">
                                {{ ($offTimeRequests->currentPage() - 1) * $offTimeRequests->perPage() + $loop->iteration }}
                            </td>
                            <td style="color: #475569; font-weight: 500;">
                                {{ $req->department ?? 'All Employees' }}
                            </td>
                            <td style="font-weight: 600; color: #1e293b;">
                                {{ $req->employee?->name ?? '-' }}
                            </td>
                            <td style="color: #475569;">
                                {{ $req->offTimeType?->name ?? 'Izin' }}
                            </td>
                            <td style="color: #475569;">
                                @if ($req->start_date && $req->end_date && $req->start_date->equalTo($req->end_date))
                                    {{ $req->start_date->format('d') }}-{{ $req->end_date->format('d F Y') }}
                            ...
                            <td style="text-align: center; position: relative;">
                                <div x-data="{ open: false }" style="display: inline-block; position: relative;">
                                    ...
                                    >
                                        @if ($req->status !== 'Approved')
                                            <form method="POST" action="{{ route('off-time-requests.update-status', $req) }}" style="display: block;">
                                                @csrf
                                                @method('PATCH')
                                        ...
                                        <form method="POST" action="{{ route('off-time-requests.destroy', $req) }}" style="display: block;" onsubmit="return confirm('Hapus pengajuan ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                                type="submit"
        ...
        {{-- PAGINATION --}}
        @if ($offTimeRequests->hasPages())
            <div style="margin-top: 24px; display: flex; justify-content: space-between; align-items: center; font-size: 13px; color: #64748b;">
                <div>
                    Menampilkan {{ $offTimeRequests->firstItem() }} sampai {{ $offTimeRequests->lastItem() }} dari {{ $offTimeRequests->total() }} data
                </div>
                <div>
                    {{ $offTimeRequests->links() }}
                </div>
            </div>
        @endif
