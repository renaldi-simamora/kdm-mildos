...
@section('title', 'Change Shift Requests')
...
@section('content')
    ...
    <div class="breadcrumb-card">
        ...
        <a href="{{ route('change-shift-requests.index') }}">Change Shift Requests</a>
        <span class="divider">/</span>
    ...
    <div class="main-card">
        ...
        {{-- FILTER BAR --}}
        <form method="GET" action="{{ route('change-shift-requests.index') }}" style="display: flex; flex-wrap: wrap; gap: 12px; align-items: center; margin-bottom: 24px;">
            {{-- Status Filter --}}
            ...
            @if ($search || ($status && $status !== 'all') || $date)
                <a href="{{ route('change-shift-requests.index') }}" class="btn-reset-gray" style="height: 38px; padding: 0 16px; display: inline-flex; align-items: center;">
                    Reset Filter
                </a>
            @endif
        </form>
        ...
        <div class="table-responsive">
            <table class="data-table" id="change-shift-requests-table">
                <thead>
                    <tr>
                ...
                <tbody>
                    @forelse ($changeShiftRequests as $index => $req)
                        <tr>
                            <td style="color: #94a3b8; font-size: 13px;">
                                {{ ($changeShiftRequests->currentPage() - 1) * $changeShiftRequests->perPage() + $loop->iteration }}
                            </td>
                            <td style="font-weight: 600; color: #1e293b;">
                                {{ $req->employee?->name ?? '-' }}
                            </td>
                            <td style="color: #475569;">
                                {{ $req->delegateEmployee?->name ?? '-' }}
                            </td>
                            <td style="color: #475569;">
                                {{ $req->shift_to ?? ($req->shift?->name ?? '-') }}
                            </td>
                            <td style="color: #475569;">
                            ...
                            <td style="text-align: center; position: relative;">
                                <div x-data="{ open: false }" style="display: inline-block; position: relative;">
                                    ...
                                    >
                                        @if ($req->status !== 'Approved')
                                            <form method="POST" action="{{ route('change-shift-requests.update-status', $req) }}" style="display: block;">
                                                @csrf
                                                @method('PATCH')
                                        ...
                                        <form method="POST" action="{{ route('change-shift-requests.destroy', $req) }}" style="display: block;" onsubmit="return confirm('Hapus pengajuan ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                                type="submit"
        ...
        {{-- PAGINATION --}}
        @if ($changeShiftRequests->hasPages())
            <div style="margin-top: 24px; display: flex; justify-content: space-between; align-items: center; font-size: 13px; color: #64748b;">
                <div>
                    Menampilkan {{ $changeShiftRequests->firstItem() }} sampai {{ $changeShiftRequests->lastItem() }} dari {{ $changeShiftRequests->total() }} data
                </div>
                <div>
                    {{ $changeShiftRequests->links() }}
                </div>
            </div>
        @endif
